...
for i in range(num_samples):
    ...
    mass_1 = np.random.uniform(5, 10) * solar_mass 
    mass_2 = np.random.uniform(1.2, 1.9) * solar_mass
    spin_1z = np.random.uniform(-0.5, 0.5)  
...
df.to_csv("/ayca/myz/data/1000_example/NSBH_1000_4K.csv", index=False)

print("Data saved in CSV format: NSBH_1000_4K.csv")
...
for i, signal in enumerate(data):
    ...
    plt.plot(signal["L1"], label=f"Sinyal {i + 1} - L1", color="purple")
    plt.legend()
plt.tight_layout()
plt.show()
